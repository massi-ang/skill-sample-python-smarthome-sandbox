...
class ApiHandlerEvent:
    def create(self, request):
        logger.info("event.create.request -----")
        logger.info(request)
        ...
        try:
            ...
            event_type = json_object["event"][
                "type"
            ]  # Expect AddOrUpdateReport, ChangeReport, DeleteReport
            endpoint_user_id = json_object["event"]["endpoint"][
                "userId"
            ]  # Expect a Profile
            endpoint_id = json_object["event"]["endpoint"][
                "id"
            ]  # Expect a valid AWS IoT Thing Name
            ...
            response = AlexaResponse(
                name="ErrorResponse", message="No valid event type"
            )
            ...
            if event_type == "AddOrUpdateReport":
                    ...
                    "endpoints": [
                        {
                            ...
                            "manufacturerName": endpoint_sku_details[
                                "manufacturer_name"
                            ],
                            "displayCategories": endpoint_sku_details[
                                "display_categories"
                            ],
                ...
                response = self.send_event(
                    "Alexa.Discovery", "AddOrUpdateReport", endpoint_id, token, payload
                )
            ...
            if event_type == "ChangeReport":
                try:
                    ...
                    if instance:
                        ...
                        prop = AlexaResponse.create_context_property(
                            namespace=namespace, name=state, value=state_value
                        )
                    ...
                    logger.info("event.create.iot_aws.update_thing_shadow.result -----")
                    logger.info(result)

                    # Update Alexa with an Event Update
                    if endpoint_user_id == "0":
                        logger.info("Event: Not sent for user_id of 0")
                    else:
                        payload = {
                            "change": {
                        ...
                        logger.info("Event: Sending event")
                        response = self.send_event(
                            "Alexa", "ChangeReport", endpoint_id, token, payload
                        )
                    ...
                    alexa_response = AlexaResponse(
                        name="ErrorResponse",
                        message=e,
                        payload={"type": "INTERNAL_ERROR", "message": e},
                    )
            ...
            if event_type == "DeleteReport":
                ...
                payload = {
                    "endpoints": [{"endpointId": endpoint_id}],
                    "scope": {"type": "BearerToken", "token": token},
                }
                response = self.send_event(
                    "Alexa.Discovery", "DeleteReport", endpoint_id, token, payload
                )

            result = response.read().decode("utf-8")
            logger.info("event.create.result -----")
            logger.info(result)
            return result
    ...
    # TODO(angmas) Improve this with a database lookup
    @staticmethod
    def get_sku_details(sku):
        """Get the SKU details based on the SKU string if not provided
        """
    ...
    def get_user_info(self, endpoint_user_id):
        logger.info("event.create.get_user_info -----")
        table = boto3.resource("dynamodb").Table(USERS_TABLE)
        result = table.get_item(
            Key={"UserId": endpoint_user_id},
            AttributesToGet=[
                "UserId",
                "AccessToken",
                "ClientId",
        ...
        if result["ResponseMetadata"]["HTTPStatusCode"] == 200:
            if "Item" in result:
                logger.info("event.create.get_user_info.Users.get_item -----")
                logger.info(str(result["Item"]))
                ...
                logger.info("event.create.send_event.token_is_expired: %s", token_is_expired)
                if token_is_expired:
                    ...
                    api_auth = ApiAuth()
                    response_refresh_token = api_auth.refresh_access_token(
                        refresh_token, client_id, client_secret, redirect_uri
                    )
                    response_refresh_token_string = (
                        response_refresh_token.read().decode("utf-8")
                    )
                    response_refresh_token_object = json.loads(
                        response_refresh_token_string
                    )
                    ...
                    logger.info("expiration_utc %s", expiration_utc)
                    ...
                    logger.info("event.create.send_event.Users.update_item: %s", str(result))
                    ...
                    logger.info("Using stored access_token: %s", access_token)
    ...
    def send_event(alexa_namespace, alexa_name, endpoint_id, token, payload):
        ...
        logger.info("api_handler_event.send_event.payload:")
        logger.info(payload)
        ...
        logger.info(
            "api_handler_event.send_event HTTP Status code: %s", str(response.getcode())
        )
        return response
