        ...
        if path == "/cb":
            ...
            self.wfile.write(
                bytes(
                    ...
                    "</head><body><center>"
                    "<h2>AuthCode: {}<br/></h2>"
                    '<h3>Starting stack creation, visit <a href="https://console.aws.amazon.com/cloudformation/home?region=us-east-1#/stacks">the console for status</a>.</h3></center></body></html>'.format(
                        auth_code
                    ),
                    "UTF-8",
                )
...
def create_stack(
    stack_name, vendor_id, client_id, client_secret, access_token, refresh_token
):
...
def main():
    ...
    url_update_account_linking = "https://api.amazonalexa.com/v1/skills/{0}/stages/development/accountLinkingClient".format(
        alexa_skill_id
    )
    ...
    request = Request(
        ...
        data=bytes(json.dumps(account_linking_request), "iso-8859-1"),
    )
    ...
    try:
        ...
        print(
            "\tSetup Account Linking Request Response Status Code:", response.getcode()
        )

    except HTTPError as e:
        print(e, e.reason, e.headers)

    enable_url = "https://alexa.amazon.com/spa/index.html#skills/beta/{}/?ref=skill_dsk_skb_ys".format(
        alexa_skill_id
    )
    ...
    user_id_object = json.loads(
        api_auth.get_user_id(access_token).read().decode("utf-8")
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if sys.argv[1] == "clean-things":
            ...
            response = iot_aws.list_things_in_thing_group(thingGroupName="Samples")
            print(response)
